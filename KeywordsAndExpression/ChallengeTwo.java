...
public class ChallengeTwo{
    ...
    public static long toMilesPerHour(double kilometersPerHour){ //10.25
        if(kilometersPerHour < 0)
            return -1;
        return Math.round (kilometersPerHour / 1.609);  //10.25 / 1.609 = 6.0
    }
    public static void printConversion (double kilometersPerHour){  //10.25
        long miles = 0;
        if(kilometersPerHour < 0){ //
            System.out.println("Invalid Value");
        }else {
            miles = toMilesPerHour(kilometersPerHour); //10.25
            System.out.println(kilometersPerHour+" km/h = "+miles+" m/h");  //6
        }
    }
}